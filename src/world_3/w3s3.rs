...
pub async fn run(
    ...
    player.set_currentscreen("w3s3".to_string());
    ...
    .await;
    // If cleared
    if player.get_cleared() == 14 {
        map.create_map_array(0, 1, 0, vec![3]).await;
    } else {
        map.create_map_array(0, 2, 0, vec![4, 3]).await;
    }
    ...
    loop {
        ...
        if *pause == false {
            ...
            if enemies.is_empty() && player.get_cleared() == 14 {
                player.add_cleared();
                item_valid = true;
                choose_open = true;
...
// Cleared starts at 14, goes to 15
