...
impl AnimatedImage {
    ...
    pub async fn new(
        ...
        if spritesheet_path.is_empty() {
            ...
            return Self {
                frame_duration: frame_duration.max(0.001),
                cols: safe_cols,
                rows: safe_rows,
                total_frames: safe_cols * safe_rows,
                loop_animation,
    ...
    #[allow(unused)]
    pub fn from_preloaded_spritesheet(
        preloaded: PreloadedAnimatedSpritesheet,
        x: f32,
        y: f32,
        width: f32,
        height: f32,
        ...
        let safe_cols = preloaded.cols.max(1);
        let safe_rows = preloaded.rows.max(1);
        let frame_width = preloaded.texture.width() / safe_cols as f32;
        let frame_height = preloaded.texture.height() / safe_rows as f32;

        Self {
            texture: preloaded.texture,
            x,
            y,
            width,
            height,
    ...
    #[allow(unused)]
    pub fn from_preloaded_gif(preloaded: PreloadedAnimatedGif, x: f32, y: f32, width: f32, height: f32, loop_animation: bool) -> Self {
        let frame_count = preloaded.frame_masks.len().max(1);
        let frame_width = preloaded.texture.width() / frame_count as f32;
        let frame_height = preloaded.texture.height();
        let frame_duration = if preloaded.frame_delays.is_empty() {
            0.1
        } else {
            preloaded.frame_delays[0]
        };
        ...
        Self {
            ...
            height,
            transparency_mask: preloaded.transparency_mask,
            frame_masks: Some(preloaded.frame_masks),
            cols: frame_count,
            rows: 1,
            current_frame: 0,
            ...
            frame_height,
            frame_duration: frame_duration.max(0.001),
            frame_durations: Some(preloaded.frame_delays),
            time_accumulated: 0.0,
            state: AnimationState::Playing,
            loop_animation,
            last_update: get_time() as f32,
    ...
    #[allow(unused)]
    pub fn set_preloaded_spritesheet(&mut self, preloaded: PreloadedAnimatedSpritesheet, frame_duration: f32, loop_animation: bool) {
        *self = Self::from_preloaded_spritesheet(preloaded, self.x, self.y, self.width, self.height, frame_duration, loop_animation);
    }

    #[allow(unused)]
    pub fn set_preloaded_gif(&mut self, preloaded: PreloadedAnimatedGif, loop_animation: bool) {
        *self = Self::from_preloaded_gif(preloaded, self.x, self.y, self.width, self.height, loop_animation);
    }

    // Create from individual frames
    #[allow(unused)]
    pub async fn from_frames(frame_paths: Vec<&str>, x: f32, y: f32, width: f32, height: f32, frame_duration: f32, loop_animation: bool) -> Self {
    ...
    #[allow(unused)]
    pub async fn from_gif(gif_path: &str, x: f32, y: f32, width: f32, height: f32, loop_animation: bool) -> Self {
            ...
            Ok(file_data) => {
                ...
                if let Some((frames, delays, width_px, height_px)) = Self::process_gif_data(&file_data) {
                    ...
                    let mut combined_image =
                        Image::gen_image_color(spritesheet_width as u16, spritesheet_height as u16, Color::new(0.0, 0.0, 0.0, 0.0));
