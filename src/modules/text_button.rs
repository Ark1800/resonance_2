...
impl TextButton {
    pub fn new(x: f32, y: f32, width: f32, height: f32, text: impl Into<String>, normal_color: Color, hover_color: Color, font_size: u16) -> Self {
        ...
        let cached_text_position = Vec2::new(x + (width / 2.0) - (cached_text_width / 2.0), y + (height / 2.0));
    ...
    pub fn with_font(&mut self, font: Font) -> &mut Self {
        ...
        self.cached_text_position = Vec2::new(self.x + (self.width / 2.0) - (self.cached_text_width / 2.0), self.y + (self.height / 2.0));
    ...
    pub fn with_text_color(&mut self, color: Color) -> &mut Self {
        ...
        if self.hover_text_color == WHITE {
            // Only update if it wasn't explicitly set
            self.hover_text_color = color;
        }
        self
    }
    ...
    pub fn click(&self) -> bool {
        ...
        let mut button_color = if self.enabled {
            if is_hovered { self.hover_color } else { self.normal_color }
        } else {
            self.off_color
        };
        ...
        if self.corner_radius > 0.0 {
            ...
            if !is_background_transparent {
                ...
                draw_round_rect_lines(draw_x, draw_y, self.width, self.height, self.corner_radius, 1.0, lowlight);
            }
            ...
            if self.border {
                draw_rectangle_lines(draw_x, draw_y, self.width, self.height, self.border_thickness, self.border_color);
            }
        }

        // Draw the text with the appropriate font using cached position
        let current_text_color = if self.enabled {
            if is_hovered { self.hover_text_color } else { self.text_color }
        } else {
