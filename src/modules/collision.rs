    ...
    U: Collidable,
{
    ...
    if mask1_opt.is_none() && mask2_opt.is_none() {
        ...
        if angle1 != 0.0 || angle2 != 0.0 {
            return check_rotated_rectangle_collision(pos1, size1, angle1, pos2, size2, angle2);
        }
    ...
    if angle1 != 0.0 || angle2 != 0.0 {
        return check_rotated_pixel_collision(obj1, obj2, &overlap_x, &overlap_y, &overlap_w, &overlap_h, skip_pixels);
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        return (0..*overlap_h as usize).into_par_iter().step_by(skip_pixels).any(|y| {
            (0..*overlap_w as usize).into_par_iter().step_by(skip_pixels).any(|x| {
                ...
                if !is_point_in_bounds(local_point1, pos1, size1) || !is_point_in_bounds(local_point2, pos2, size2) {
                    return false;
                }
...
fn check_rotated_rectangle_collision(pos1: Vec2, size1: Vec2, angle1: f32, pos2: Vec2, size2: Vec2, angle2: f32) -> bool {
