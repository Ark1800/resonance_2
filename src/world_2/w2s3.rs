...
pub async fn run(
    ...
    player.set_currentscreen("w2s3".to_string());
    let mut map = Map::new(
        virtual_width,
        virtual_height,
    ...
    for _i in 0..3 {
        let mut mage = Enemy::new("", 50.0, 50.0, magex, 100.0, true, 1.0, 56.0, 32.0, "", "mage").await;
        ...
        magex += 300.0; // Adjust the x position for the next mage
        enemies.push(mage);
    }
    for _i in 0..2 {
        let mut summoner: Enemy = Enemy::new("", 50.0, 50.0, 500.0, 300.0, true, 1.0, 47.0, 23.0, "", "summoner").await;
